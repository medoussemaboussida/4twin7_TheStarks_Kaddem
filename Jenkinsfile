...
pipeline {
    ...
    stages {
        ...
        stage('Deploy to Nexus') {
            steps {
                script {
                    def artifactUrl = 'http://192.168.50.4:8081/repository/maven-releases/tn/esprit/spring/kaddem/0.0.1/kaddem-0.0.1.jar'
                    def exists = sh(script: "curl --head --silent --fail ${artifactUrl}", returnStatus: true) == 0
                    if (exists) {
